...
export type Employee = {
  ...
  email?: string; extensionNumber?: string; departmentId?: number; departmentName?: string; role: "USER" | "NOTICE_WRITER" | "ADMIN";
  status: string; online: boolean; statusMessage?: string; availability?: "ONLINE" | "BUSY" | "AWAY" | "OFFLINE";
...
export type Notice = {
  id: number; senderId: number; senderName: string; senderPosition?: string; senderDepartment?: string;
  title: string; content?: string; recipients: string[]; sentAt: string; read: boolean; file?: FileInfo;
};
...
export async function fetchAttachment(downloadUrl: string): Promise<Blob> {
  ...
  const response = await fetch(new URL(downloadUrl, apiUrl.origin), { headers });
...
export const api = {
  ...
  updateMe: (data: Record<string, unknown>) =>
    request<Employee>("/auth/me", { method: "PUT", body: JSON.stringify(data) }),
  ...
  createRoom: (name: string, employeeIds: number[]) =>
    request<Room>("/rooms", { method: "POST", body: JSON.stringify({ name, employeeIds, type: employeeIds.length === 1 ? "DIRECT" : "GROUP" }) }),
  ...
  updateEmployee: (employeeId: number, data: Record<string, unknown>) =>
    request<Employee>(`/admin/employees/${employeeId}`, { method: "PUT", body: JSON.stringify(data) }),
  ...
  sendNotice: (title: string, content: string, recipientIds: number[], file?: File) => {
